...
(function($, undefined) {
    ...
    var jalaliparseDate = function(value, formats, culture) {
        if (value && value.length > 0 && formats && formats.indexOf("yyyy/MM/dd") > -1) {
            if(value !== "____/__/__"){
    ...
    ui = kendo.ui,
    Widget = ui.Widget,
    parse = jalaliparseDate,
    keys = kendo.keys,
    template = kendo.template,
    activeElement = kendo._activeElement,
    DIV = "<div />",
    SPAN = "<span />",
    ns = ".kendoJalaliDatePicker",
    CLICK = "click" + ns,
    OPEN = "open",
    CLOSE = "close",
    CHANGE = "change",
    ...
    DateView.prototype = {
        _calendar: function() {
            ...
            if (!calendar) {
                ...
                that.calendar = calendar = new ui.JalaliCalendar(div);
                that._setOptions(options);

                kendo.jalaliCalendar.makeUnselectable(calendar.element);
    ...
    var JalaliDatePicker = Widget.extend({
        ...
        _option: function(option, value) {
            ...
            options[option] = new JalaliDate(+value);
...
(function ($) {
    ...
    var MaskedJalaliDatePicker = Widget.extend({
        init: function (element, options) {
            var that = this;
            Widget.fn.init.call(this, element, options);

            $(element).kendoMaskedTextBox({ mask: that.options.dateOptions.mask || "0000/00/00" })
            .kendoJalaliDatePicker({
                format: that.options.dateOptions.format || "yyyy/MM/dd",
                parseFormats: that.options.dateOptions.parseFormats || ["yyyy/MM/dd", "yyyy/MM/dd"]
            })
        ...
        value: function (value) {
            var datepicker = this.element.data("kendoJalaliDatePicker");
    ...
    ui.plugin(MaskedJalaliDatePicker);
})(window.kendo.jQuery);